import { HttpClient } from '@angular/common/http';
import { TestBed } from '@angular/core/testing';
import { of } from 'rxjs';
import { Gender } from '../shared/gender.model';
import { Person } from './person.model';
import { PersonService } from './person.service';

const personData: Person = {
  _id: '1',
  name: 'Lucas Kok',
  birthday: new Date('09-01-2005'),
  deathday: new Date(),
  gender: Gender.male,
  imageBase64:
    '',
};

const personsDataArray: Person[] = [
  {
    _id: '1',
    name: 'Lucas Kok',
    birthday: new Date('09-01-2005'),
    deathday: new Date(),
    gender: Gender.male,
    imageBase64:
      '',
  },
  {
    _id: '2',
    name: 'Chevy Rietveld',
    birthday: new Date('24-11-2006'),
    deathday: new Date(),
    gender: Gender.male,
    imageBase64:
      '',
  },
  {
    _id: '3',
    name: 'Rik Vandermullen',
    birthday: new Date('17-09-1653'),
    deathday: new Date(),
    gender: Gender.male,
    imageBase64:
      '',
  },
];

describe('PersonService', () => {
  let service: PersonService;
  let httpSpy: jasmine.SpyObj<HttpClient>;

  beforeEach(() => {
    httpSpy = jasmine.createSpyObj('HttpClient', [
      'get',
      'post',
      'put',
      'delete',
    ]);

    TestBed.configureTestingModule({
      providers: [{ provide: HttpClient, useValue: httpSpy }],
    });

    service = TestBed.inject(PersonService);
    httpSpy = TestBed.inject(HttpClient) as jasmine.SpyObj<HttpClient>;
  });

  it('should be created', () => {
    expect(service).toBeTruthy();
  });

  it('should create a Person and return it', (done: DoneFn) => {
    httpSpy.post.and.returnValue(of(personData));

    service.addPerson(personData, 'base64image').subscribe((artist) => {
      expect(artist);
      expect(artist._id).toEqual(personData._id!);
      done();
    });
  });

  it('should return a Person', (done: DoneFn) => {
    httpSpy.get.and.returnValue(of(personData));

    const jwtToken = 'jwtToken';
    service.getPersonById('2', jwtToken).subscribe((artist) => {
      expect(artist);
      expect(artist._id).toEqual(personData._id!);
      done();
    });
  });

  it('should return a Person Array', (done: DoneFn) => {
    httpSpy.get.and.returnValue(of(personsDataArray));

    const jwtToken = 'jwtToken';
    service.getAllPersonsFromUser(jwtToken).subscribe((persons: Person[]) => {
      expect(persons);
      expect(persons.length).toBe(3);
      expect(persons[0]._id).toEqual(personsDataArray[0]._id!);
      done();
    });
  });

  it('should update a Person and return it', (done: DoneFn) => {
    httpSpy.put.and.returnValue(of(personData));

    const jwtToken = 'jwtToken';
    service.updatePerson(personData, jwtToken).subscribe((artist) => {
      expect(artist);
      expect(artist._id).toEqual(personData._id!);
      done();
    });
  });
});
